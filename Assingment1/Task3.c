    ...
    /* Deal with collision */
        ...
        while ( hashTable[index].frequency != 0 ){ 
            ...
            if(strcmp(hashTable[index].name, s) == 0 )
                break;
            if ((index + hash2(s)) < HASH_TABLE_SIZE)
                    index += hash2(s);
            else
                index = (index + hash2(s)) - HASH_TABLE_SIZE; // If we reach the end of the array 
        ...
        if (hashTable[index].frequency != 0)
            if (strcmp(hashTable[index].name, userInput) == 0)
                ...
                while(strcmp(hashTable[index].name, userInput) != 0) {
                    if (hashTable[index].frequency == 0)
                        break;
                    if ((index + hash2(userInput) < HASH_TABLE_SIZE))
                        index += hash2(userInput);
                    else
                        index = (index + hash2(userInput)) - HASH_TABLE_SIZE; // If we reach the end of the array 
                }
                printf(">> %s %d\n", hashTable[index].name, hashTable[index].frequency);
            }
        else
